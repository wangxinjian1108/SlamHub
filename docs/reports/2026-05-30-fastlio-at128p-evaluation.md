...
| Backend | 状态 | 路径长度 (m) | Z drift (std) | 备注 |
|---------|------|--------------|---------------|------|
| FAST-LIO2 | ✅ 生产 | 452.70 | 2.00 | ESKF + IKD-tree，自估 gravity |
| KISS-ICP  | ✅ 生产（§14/§15）| 与 FL 一致 | 与 FL 一致 | LiDAR-only，最稳定 |
| **LIO-SAM** | ✅ 跑通 (§17) | **454.27** | **1.47** | 因子图 + loop closure，Z drift 反而最小 |

LIO-SAM 的 Z std 比 FAST-LIO 小 ~26%，应是 loop closure + ISAM2 优化全图的效果。
路径长度三家在 ±0.4% 内一致。
...
| `scripts/eval_liosam_vs_fastlio.py`    | Umeyama 对齐 + ATE/RPE/Z drift |
...
| KISS-ICP benchmark | ✅ landed (§14/§15) | 默认生产 backend |
| **LIO-SAM benchmark** | ✅ **landed (§17)** | 三方对比可比性已就位 |
| RPE + alarms | ✅ landed (§13) | |
| 多 sample 稳定性 | 🔜 next | 当前仅单段 60s 数据 |
| Loop closure（如果需要长 SLAM） | ✅ 已经有了 | LIO-SAM 自带 |
| D1 联合 BA | 🔜 | 长期方向 |
