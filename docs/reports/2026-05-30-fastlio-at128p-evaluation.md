...
## 20. 多 sample 稳定性验证（3 sample × 7 backend）

§14-§19 全部基于一段 60 s 录制（ZL11626）。本节用同样的 7-way pipeline 跑
另外 2 段录制：
...
7. FAST-LIVO2 — GHCR 镜像，LiDAR + IMU + camera VIO，per-recording config via `gen_fastlivo2_config.py`
...
| Sample / Secondary | FAST-LIO | KISS-ICP | LIO-SAM | LIO-SAM\* | GenZ-ICP | MAD-ICP | LIVO2 |
|--------------------|---------:|---------:|--------:|----------:|---------:|--------:|------:|
| **ZL11626** flash_front | 0.191 | 0.128 | 0.531 | 0.135 | **0.125** | 0.159 | 2.089 |
| ZL11626 flash_rear      | 0.598 | 0.286 | 0.546 | 0.241 | 0.269 | **0.198** | 3.852 |
| ZL11626 rfr             | 0.382 | 0.311 | 0.809 | 0.306 | 0.306 | **0.301** | 1.330 |
| **ZL10359** flash_front | 0.884 | 1.311 | **0.497** | 1.448 | 1.478 | 1.444 | 1.679 |
| ZL10359 flash_rear      | 1.436 | 1.045 | **0.814** | 1.118 | 1.137 | 1.340 | 7.012 |
| ZL10359 rfr             | 5.033 | 4.463 | 4.060 | 5.466 | 4.241 | **3.951** | 5.661 |
| **ZL10966** flash_front | 0.130 | **0.129** | 0.192 | 0.283 | 0.166 | 0.183 | 0.337 |
| ZL10966 flash_rear      | 0.374 | 0.373 | 0.528 | 0.648 | 0.567 | **0.317** | 0.617 |
| ZL10966 rfr             | 0.134 | 0.111 | 0.199 | 0.249 | **0.110** | 0.126 | 0.162 |
...
| 路径 | 内容 |
|------|------|
| `scripts/run_all_backends.sh` | 一键跑全（7 backend + B2，可中断 resume） |
...
| **FAST-LIVO2 benchmark** | ✅ **landed** | 7th backend（§22 LIVO2 补丁），表现垫底 |
...
`scripts/eval_internal_quality.py` 算每个 (sample, secondary) 上 7 个 backend
估出的 `translation_xyz_m` 的 std。如果 6-7 个差异巨大的 SLAM 都收敛到同一个
...
| Sample | Sec | FAST-LIO | KISS-ICP | LIO-SAM | LIO-SAM\* | GenZ-ICP | MAD-ICP | LIVO2 |
|--------|-----|---------:|---------:|--------:|----------:|---------:|--------:|------:|
| ZL11626 | flash_front | 0.191 | 0.128 | 0.531 | 0.135 | **0.125** | 0.159 | 2.089 |
| ZL11626 | flash_rear  | 0.598 | 0.286 | 0.546 | 0.241 | 0.269 | **0.198** | 3.852 |
| ZL11626 | rfr         | 0.382 | 0.311 | 0.809 | 0.306 | 0.306 | **0.301** | 1.330 |
...
| ZL10966 | flash_rear  | 0.374 | 0.373 | 0.528 | 0.648 | 0.567 | **0.317** | 0.617 |
| ZL10966 | rfr         | 0.134 | 0.111 | 0.199 | 0.249 | **0.110** | 0.126 | 0.162 |
| ZL10968 | flash_front | 0.102 | **0.043** | 0.298 | 0.049 | 0.045 | 0.055 | 0.821 |
| ZL10968 | flash_rear  | 0.346 | 0.170 | 0.378 | **0.083** | 0.171 | 0.104 | 1.336 |
| ZL10968 | rfr         | 0.216 | 0.174 | 0.447 | 0.191 | 0.163 | **0.138** | 0.495 |
| ZL11881 | flash_front | 0.542 | 0.407 | **0.193** | 0.325 | 0.375 | 0.369 | 0.702 |
| ZL11881 | flash_rear  | 0.193 | 0.105 | 0.201 | 0.179 | **0.085** | 0.122 | 0.977 |
| ZL11881 | rfr         | 0.198 | 0.119 | 0.261 | **0.107** | 0.126 | 0.118 | 0.162 |
| ZL12332 | flash_front | **0.102** | 0.185 | 0.157 | 0.135 | 0.150 | 0.205 | 0.125 |
| ZL12332 | flash_rear  | 0.356 | 0.287 | 0.246 | 0.319 | **0.229** | 0.281 | 0.282 |
| ZL12332 | rfr         | 0.053 | 0.037 | 0.170 | 0.040 | **0.026** | 0.030 | 0.080 |
| ZL12382 | flash_front | 1.097 | 1.233 | 1.636 | **0.980** | 1.060 | 1.093 | 2.117 |
| ZL12382 | flash_rear  | 2.411 | 3.002 | 1.602 | **1.247** | 2.126 | 1.255 | 4.633 |
| ZL12382 | rfr         | **0.234** | 0.243 | 0.242 | 0.372 | 0.240 | 0.256 | 0.214 |
...
| FAST-LIO | 2 |
| KISS-ICP | 2 |
| LIVO2    | 1 (ZL12382/rfr: 0.214 m) |

**§20 的"MAD-ICP 4/9"在扩到 7 样本后变成"MAD-ICP / GenZ-ICP 各 5/21"，
两者打平**。3 样本的优势统计学不显著。LIO-SAM\* 在难样本（ZL10359 / ZL12382）
上崛起 —— 因子图 + IMU 在低信号场景下补几何信息。LIVO2 仅在 ZL12382/rfr 上
胜出（0.214 m vs FAST-LIO 0.234 m），其余 20 个 cell 均排名最末或接近最末。

### 21.4 跨 backend 一致性（GT-free，最重要的信号）

`scripts/eval_internal_quality.py` 输出 21 个 (sample, secondary) 的 7-backend
xyz spread（std_norm 单位 m，越小=backends 越一致 = 更可信）：

| Sample | flash_front spread | flash_rear | rfr | 数据特征 |
|--------|-------------------:|-----------:|----:|----------|
| **ZL12332** | **0.048** | 0.113 | 0.066 | 长 + 慢，但路径多变 → 最一致 |
| **ZL10966** | **0.094** | 0.219 | **0.092** | 短但中速，多方向 |
| **ZL11881** | 0.195 | 0.356 | **0.066** | 中段，标准 |
| **ZL10968** | 0.326 | 0.545 | 0.264 | 中长，5 m/s |
| **ZL11626** | 0.807 | 1.448 | 0.630 | 长 + 快（baseline）|
| ZL12382 | 0.564 | **1.284** | 0.074 | 几乎不动 → flash_rear 严重分歧 |
| ZL10359 | 0.567 | 2.123 | **0.753** | 短 + 慢 + 单方向 → 全方位分歧 |
...
| LIVO2    | x0.797 y0.442 z0.067 | x1.485 y2.528 z0.144 | x1.396 y1.359 z0.074 | ~8.3 |
...
| `output/multi_sample/{ZL10968,ZL11881,ZL12332,ZL12382}/` | 4 段新样本，每段 7 backend 完整结果 |
...
| **FAST-LIVO2 benchmark** | ✅ **landed (§22)** | 7th backend; 1/21 wins, +0.9 m Δx bias — 不推荐用于 cross-LiDAR 标定 |
...
## 22. 逐项校准值差异：7 backend × 7 sample × 3 secondary side-by-side
...
(sample, secondary) cell × 7 backend × 6 维 = 882 个数）。本节只摘四个
...
| 路径 | 内容 |
|------|------|
| `scripts/eval_calib_side_by_side.py` | 逐 cell 摊开 7×6 维度的诊断 |
...
| `output/multi_sample/calib_backend_bias.png` | 7 个 backend 的 axis-wise 系统 bias（LIVO2 为明显 outlier）|
...
|---------|--------:|-------:|--------:|-------:|--------:|-------:|
| FAST-LIO | -0.074 | 0.146 | +0.047 | 0.215 | -0.006 | 0.020 |
| KISS-ICP | +0.015 | 0.159 | +0.042 | 0.245 | +0.005 | 0.016 |
| LIO-SAM  | **-0.118** | **0.231** | **-0.150** | **0.282** | -0.004 | 0.029 |
| LIO-SAM\* | +0.065 | 0.252 | -0.016 | 0.098 | -0.004 | 0.020 |
| **GenZ-ICP** | -0.040 | **0.125** | +0.037 | **0.116** | +0.001 | **0.011** |
| MAD-ICP  | -0.079 | 0.266 | +0.021 | 0.051 | -0.003 | 0.007 |
| LIVO2    | **+0.905** | **0.958** | +0.102 | **1.368** | +0.054 | **0.093** |

**三个 takeaway**：

1. **GenZ-ICP 跨 21 cell 是离 median 最近的 backend**（Δx std 12.5 cm，
   Δy std 11.6 cm 都是最低）—— 没有 GT 时它最像"群体共识"。
2. **LIO-SAM (feature-only) Δy 差 15 cm** + std 28 cm，远大于其他 backend。
   LIO-SAM\* hybrid 把 Δy std 压到 10 cm —— map quality 是关键。
3. **LIVO2 Δx mean +0.91 m, std 0.96 m** —— 系统性偏移一个数量级大于所有
   其他 backend。原因：camera VIO 引入的 extrinsic 与 AT128P 4-coef 畸变模型
   不完全匹配（丢 k3），加上 image-lidar 时间同步 jitter 导致 scale drift。
   LIVO2 在 cross-LiDAR 标定任务上不如纯 LiDAR / LiDAR-IMU backends。
4. **所有非 LIVO2 backend Δz 都很小**（≤ 2.9 cm std），证明 cross-LiDAR 标定的 z
...
2026-05-31，§13/§14/§15 增补于 2026-05-31，§16 增补于 2026-06-01，§17 增补于
2026-06-02，§18 增补于 2026-06-02，§19 增补于 2026-06-02，§20 增补于 2026-06-02，
§21 增补于 2026-06-03，§22 增补于 2026-06-03，§20–§22 LIVO2 增补于 2026-06-09。_
